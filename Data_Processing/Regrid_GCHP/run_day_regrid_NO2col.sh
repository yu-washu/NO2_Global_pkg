...
SCRIPT="/my-projects2/1.project/NO2_DL_global/NO2_global_pkg/Data_Processing/Regrid_GCHP/regen_NO2col.py"
LOGDIR="logs_regrid_NO2col"; mkdir -p "$LOGDIR"
...
while [[ "$d" < "$END" || "$d" == "$END" ]]; do
  ...
  bsub -q rvmartin \
       ...
       -R "select[hname!='compute1-exec-10.ris.wustl.edu']" \
       -R "select[hname!='compute1-exec-11.ris.wustl.edu']" \
       -R "select[hname!='compute1-exec-12.ris.wustl.edu']" \
       -R "select[port8543=1]" \
       -R "rusage[mem=${MEM}]" \
...
/bin/bash && \
ulimit -s unlimited && \
exec >\"${LOGDIR}/Regrid_${DATE}.out\" 2>&1
echo \"[\$(hostname)] Regriding ${DATE}\"
