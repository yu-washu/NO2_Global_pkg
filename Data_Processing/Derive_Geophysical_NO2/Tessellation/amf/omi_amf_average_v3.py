...
def average_daily_to_monthly(year, month):
    base_dir    = f'/my-projects2/1.project/NO2col-v3/OMI_KNMI/{year}'
    ...
    for day in range(1, days_in_month + 1):
        fname = f"OMI_KNMI_AMFtot_Regrid_{year}{month:02d}{day:02d}_{qcstr}.nc"
    ...
    try:
        ...
        monthly_mean.attrs.update({
            'title': f'Monthly mean OMI-KNMI AMF for {year}-{month:02d}',
            'source': 'tess_OMI_KNMI_AMFtot.py daily output',
        ...
        out_fname = f"OMI_KNMI_AMFtot_{year}{month:02d}_Monthly_{qcstr}.nc"
...
def average_monthly_to_yearly(year):
    ...
    for month in range(1, 13):
        fname = f"OMI_KNMI_AMFtot_{year}{month:02d}_Monthly_{qcstr}.nc"
    ...
    try:
        ...
        yearly_mean.attrs.update({
            'title': f'Yearly mean OMI-KNMI AMF for {year}',
            'source': 'Monthly AMF averages',
            'created': datetime.now().isoformat(),
            'months_included': str(months_found),
        })

        out_fname = f"OMI_KNMI_AMFtot_{year}_Yearly_{qcstr}.nc"
...
def plot_amf(nc_path, title, out_png):
    ...
    # invert=True means plot 1/var instead of var directly
    panels = [
        ('amf_trop',   'amf_trop',                     'AMF (dimensionless)',   'RdYlBu_r', 1.0, 4.0,  False),
        ('amf_trop_gcshape',  'amf_trop_gcshape',  'AMF (dimensionless)', 'RdYlBu_r', 1.0, 4.0,  False),
        ('amf_tot', 'amf_tot',              'AMF (dimensionless)', 'RdYlBu_r',  1.0, 5.0,  False),
        ('amf_tot_gcshape', 'amf_tot_gcshape', 'AMF (dimensionless)', 'RdYlBu_r', 1.0, 5.0,  False),
    ]
    ...
    for ax, (var, label, unit, cmap, vmin, vmax, invert) in zip(axes, panels):
        if var not in ds:
        ...
        data = ds[var].values
        if invert:
            data = np.where(data != 0, 1.0 / data, np.nan)
...
def main():
    parser = argparse.ArgumentParser(
        description='Monthly/yearly average of OMI-KNMI AMF_tot and AMF_gcshape'
    )
    ...
    print(f"=== OMI-KNMI AMF AVERAGING ===", flush=True)
    ...
    if args.yearly_only:
        ...
        yearly_dir  = os.path.join(base_dir, 'yearly_AMFtot')
        os.makedirs(yearly_dir, exist_ok=True)
        yearly_fname = f"OMI_KNMI_AMFtot_{year}_Yearly_{qcstr}.nc"
        ...
        if args.plot_only:
            ...
            if nc_path:
                ...
                plot_amf(nc_path, f"OMI-KNMI AMF — {year} Annual Mean", out_png)
        else:
            nc_path = average_monthly_to_yearly(year)
        ...
        monthly_dir = os.path.join(base_dir, 'monthly_AMFtot')
        os.makedirs(monthly_dir, exist_ok=True)
        monthly_fname = f"OMI_KNMI_AMFtot_{year}{args.month:02d}_Monthly_{qcstr}.nc"
        ...
        if args.plot_only:
            ...
            if nc_path:
                out_png = nc_path.replace('.nc', '.png')
                plot_amf(nc_path, f"OMI-KNMI AMF — {year}-{args.month:02d} Monthly Mean", out_png)
            return
        else:
            nc_path = average_daily_to_monthly(year, args.month)
            if nc_path and not args.no_plot:
                out_png = nc_path.replace('.nc', '.png')
                plot_amf(nc_path,
                        f"OMI-KNMI AMF — {year}-{args.month:02d} Monthly Mean",
                        out_png)
