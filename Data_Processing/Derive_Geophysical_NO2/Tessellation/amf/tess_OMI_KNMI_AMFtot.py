...
tess_var_str = ['omi_AMFtot', 'omi_AMF_gcshape']

# ─── PER-DAY WORKER ──────────────────────────────────────────────────────────────
def process_single_day(year, month, day):
    out_dir            = f'/my-projects2/1.project/NO2_col/OMI_KNMI-v2/{year}/daily_AMFtot/'
    ...
    try:
        ...
        for i, fname in enumerate(files, start=1):
            ...
            amf_gcshape = np.full(omi['AMFtot'].shape, np.nan)
            ...
            for idx, (j, k) in enumerate(zip(*valid)):
                ...
                AvK    = omi['AvKtot'][0, si, gi, :]

                # DIAGNOSTIC: print normalization of AvK for the first 3 good pixels
                # if idx < 3:
                #     print(f"  [DIAG idx={idx}] sum(AvK)={np.sum(AvK):.4f}  AMFtot={AMFtot:.4f}  "
                #           f"sum(AvK)/AMFtot={np.sum(AvK)/AMFtot:.4f}  "
                #           f"sum(prof*AvK)/sum(prof)={np.sum(prof_tm5*AvK)/np.sum(prof_tm5):.4f}",
                #           flush=True)

                denom  = AMFtot * np.sum(prof_tm5 * AvK)

                if denom and not np.isnan(denom):
                    amf_gcshape[0, j, k] = denom / np.sum(prof_tm5)
                    
            gcshape_amf = amf_gcshape[good].copy()
            gcshape_amf[np.isnan(gcshape_amf)] = 0.0
