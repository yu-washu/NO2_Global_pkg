"""
Map the number of daily samples (valid days) per pixel for TROPOMI NO2.
...
DEFAULT_BASE_DIR = '/my-projects2/1.project/NO2_col/TROPOMI/'
...
def get_sample_count(year, month=None, base_dir=DEFAULT_BASE_DIR, var='NO2_tot'):
    ...
    for m in month_list:
        ...
        for day in range(1, days_in_month + 1):
            fname = f"Tropomi_Regrid_{year}{m:02d}{day:02d}_{qcstr}.nc"
...
def main():
    parser = argparse.ArgumentParser(
        description='Map number of daily samples per pixel for TROPOMI NO2'
    )
    ...
    parser.add_argument('--base-dir', type=str, default=DEFAULT_BASE_DIR,
                        help='Base directory for TROPOMI daily/yearly')
    ...
    if args.out_nc:
        ...
        ds_out = xr.Dataset(
            ...
            attrs={
                'title': f'TROPOMI daily sample count per pixel, {period_str}',
                'period': period_str,
    ...
    if args.out_png:
        ...
        out_png = os.path.join(subdir, f"TROPOMI_sample_count_{suffix}_{qcstr}.png")
