...
def process_single_day(year, month, day):
    out_dir            = f'/my-projects2/1.project/NO2col-v3/OMI_KNMI/{year}/daily/'
    ...
    try:
        ...
        for i, fname in enumerate(files, start=1):
            ...
            omi['no2_trop_vc'] = np.where(amf_bad, np.nan, omi['no2_trop_vc'])
            omi['no2_tot_vc'] = np.where(amf_bad, np.nan, omi['no2_tot_vc'])
