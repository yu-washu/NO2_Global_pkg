...
SCRIPT="/my-projects2/1.project/NO2_DL_global/NO2_global_pkg/Data_Processing/Derive_Geophysical_NO2/Tessellation/tess/tess_OMI_KNMI_v2.py"
...
while [[ "$d" < "$END" || "$d" == "$END" ]]; do
  ...
  echo "DEBUG: submitting Tessellation${DATE}"
  bsub -q rvmartin \
       -J "Tess${DATE}" \
       -g "$GROUP" \
       -n 1 \
       ...
       -R "select[hname!='compute1-exec-101.ris.wustl.edu']" \
...
exec >\"${LOGDIR}/Tess_${DATE}.out\" 2>&1
echo \"[\$(hostname)] Tessellating ${DATE}\"
python3 ${SCRIPT} --year ${DATE:0:4} --mon ${DATE:4:2} --day ${DATE:6:2}
"
