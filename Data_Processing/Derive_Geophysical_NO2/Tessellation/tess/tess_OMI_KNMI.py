...
def process_single_day(year, month, day):
    out_dir            = f'/my-projects2/1.project/NO2_col/OMI_KNMI-v2/{year}/daily/'
        ...
        for i, fname in enumerate(files, start=1):
            ...
            for idx, (j, k) in enumerate(zip(*valid)):
                ...
                if denom and not np.isnan(denom):
                    no2_gcshape[0, j, k] = (sc_tot * np.sum(prof_tm5)) / denom
            ...
            gcshape_vals = no2_gcshape[good].copy()
            gcshape_vals[np.isnan(gcshape_vals)] = 0.0
            ...
            np.savetxt(
                orbit_dat,
                arr,
                fmt='%10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %15.5E %15.5E %15.5E'
            )
