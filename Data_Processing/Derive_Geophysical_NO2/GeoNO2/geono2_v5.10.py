"""
geono2_v5.10.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.10: Same bounds as v5.4 [2e-17, 1e-15] but with CLAMP instead of clip-to-NaN.
...
# Directories — v5.10: NO2col-v3 (AMF > 0.5 filter)
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.10/{year}/'


# ---------------------------------------------------------------------------
# Seasonal eta_trop clamping bounds
# ---------------------------------------------------------------------------
# Same bounds as v5.4 — but clamped instead of clipped to NaN
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.10 — daily GeoNO2 + eta_trop clamping (v5.4 bounds)',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.10 (daily GeoNO2 + eta_trop clamping)')
    ...
    print(f"geono2_v5.10  year={args.year}  instrument={args.instrument}")
