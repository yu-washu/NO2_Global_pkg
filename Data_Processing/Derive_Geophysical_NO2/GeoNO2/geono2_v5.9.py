"""
geono2_v5.9.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.9: Same bounds as v5.3 [1e-17, 2e-15] but with CLAMP instead of clip-to-NaN.
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.9/{year}/'


# ---------------------------------------------------------------------------
# Seasonal eta_trop clamping bounds
# ---------------------------------------------------------------------------
# Same bounds as v5.3 — but clamped instead of clipped to NaN
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.9 — daily GeoNO2 + eta_trop clamping (v5.3 bounds)',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.9 (daily GeoNO2 + eta_trop clamping)')
    ...
    print(f"geono2_v5.9  year={args.year}  instrument={args.instrument}")
