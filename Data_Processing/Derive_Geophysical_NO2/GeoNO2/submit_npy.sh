...
SCRIPT="/my-projects2/1.project/NO2_DL_global/NO2_global_pkg/Data_Processing/Derive_Geophysical_NO2/GeoNO2/save_npy_v5_clamp.py"
LOGDIR="logs_npy_v5.13"; mkdir -p "$LOGDIR"
...
for year in $(seq $START_YEAR $END_YEAR); do
  for month in {1..12}; do
    ...
    bsub -q general \
         ...
         -R "select[hname!='compute1-exec-100.ris.wustl.edu']" \
         -R "select[hname!='compute1-exec-102.ris.wustl.edu']" \
         -R "select[hname!='compute1-exec-103.ris.wustl.edu']" \
         -R "select[port8543=1]" \
         -R "span[ptile=${PTILE}]" \
         -R "rusage[mem=${MEM}]" \
...
exec >\"${LOGDIR}/npy_${year}${MONTH_STR}.out\" 2>&1
echo \"[\$(hostname)] Processing npy: ${year}-${MONTH_STR}\"
echo \"Start time: \$(date)\"
echo \"Job ID: \$LSB_JOBID\"
ulimit -s unlimited
python3 -u ${SCRIPT} ${year} --version v5.13 --month ${month}
echo \"End time: \$(date)\"
