"""
geono2_v5.3.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.3: Daily GeoNO2 computation with eta_trop clipping.
  ...
  constant bounds [1e-17, 2e-15]), average daily GeoNO2 to monthly, then gap-fill.
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.3/{year}/'
...
ETA_TROP_MIN = 1e-17           # constant year-round
ETA_TROP_MAX = 2e-15           # constant year-round
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.3 — daily GeoNO2 + eta_trop clipping',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.3 (daily GeoNO2 + eta_trop clipping + eta_trop clipping)')
    ...
    print(f"geono2_v5.3  year={args.year}  instrument={args.instrument}")
