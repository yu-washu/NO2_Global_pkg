...
START_YEAR=2023
END_YEAR=2023
MEM=350000           # in MB (≈350 GB) — some months exceed 300 GB (observed 319 GB peak)
SCRIPT="/my-projects2/1.project/NO2_DL_global/NO2_global_pkg/Data_Processing/Derive_Geophysical_NO2/GeoNO2/geono2_v5.3.py"
...
for year in $(seq $START_YEAR $END_YEAR); do
  for month in {1..1}; do
    ...
    JOB_NAME="GeoNO2_v5.3_${year}${MONTH_STR}"
    ...
    bsub -q general \
         ...
         -o "${LOGDIR}/plot_GeoNO2_v5.3_${year}_${MONTH_STR}.out" \
         bash -lc $'\n'"\
. /opt/conda/bin/activate && \
/bin/bash && \
cd /my-projects2/1.project/NO2_DL_global/NO2_global_pkg/Data_Processing/Derive_Geophysical_NO2/GeoNO2 && \
echo \"[\$(hostname)] Deriving GeoNO2 monthly average: ${year}-${MONTH_STR}\"
