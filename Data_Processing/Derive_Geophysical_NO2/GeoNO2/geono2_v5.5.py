"""
geono2_v5.5.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.5: Same as v5.3 but with tighter eta_trop clipping bounds [5e-17, 8e-16] (approx 5th-95th percentile of eta_trop distribution in 2023 Jan).
  Core change from v5.1: instead of computing monthly_eta from monthly means,
  compute daily GeoNO2 = daily_SatCol × daily_eta (with eta_trop clipped to
  constant bounds [5e-17, 8e-16]), average daily GeoNO2 to monthly, then gap-fill.

  Satellite data source: NO2col-v3 (AMF > 0.5 filter applied at tessellation).

  eta_trop clipping: values outside [ETA_TROP_MIN, ETA_TROP_MAX] set to NaN.
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.5/{year}/'


# ---------------------------------------------------------------------------
# Seasonal eta_trop clipping bounds
# ---------------------------------------------------------------------------
# Cosine interpolation: Jan peak (winter) → Jul trough (summer)
ETA_TROP_MIN = 5e-17           # constant year-round
ETA_TROP_MAX = 8e-16           # constant year-round
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.5 — daily GeoNO2 + tighter eta_trop clipping [5e-17, 8e-16]',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.5 (daily GeoNO2 + eta_trop clipping + eta_trop clipping)')
    ...
    print(f"geono2_v5.5  year={args.year}  instrument={args.instrument}")
