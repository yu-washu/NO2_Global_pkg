"""
geono2_v5.14.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.14: Per-month adaptive eta_trop CLAMPING using [1st, 99th] percentiles
  derived from v5.3 monthly-mean gchp_eta_trop grids. Same bounds as v5.8 but
  with clamp (np.clip) instead of clip-to-NaN, for both daily and monthly eta.

  Satellite data source: NO2col-v3 (AMF > 0.5 filter applied at tessellation).

  eta_trop clamping: values outside per-month [p1, p99] clamped to bounds.
...
# Directories — v5.14: NO2col-v3 (AMF > 0.5 filter)
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.14/{year}/'


# ---------------------------------------------------------------------------
# Per-month adaptive eta_trop clamping bounds [1st, 99th percentile]
# ---------------------------------------------------------------------------
# Read from v5.8 monthly-mean gchp_eta_trop grids at startup.
...
def get_eta_trop_bounds(month, year=2023):
    ...
    print(f"  [v5.14] Month {month:02d} eta_trop bounds from v5.3: "
...
def process_single_day(year, month, day, instrument):
    ...
    if daily_eta_trop is not None:
        ...
        valid_mask = np.isfinite(daily_eta_trop)
        valid_before = int(valid_mask.sum())
        n_clamped = int(((daily_eta_trop < eta_min) | (daily_eta_trop > eta_max)) [valid_mask].sum())
        daily_eta_trop = np.where(
            valid_mask, np.clip(daily_eta_trop, eta_min, eta_max), np.nan
        ).astype('float32')
        pct = 100.0 * n_clamped / valid_before if valid_before > 0 else 0.0
        eta_clip_stats = (valid_before, n_clamped, pct)
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    # v5.14: Monthly eta is CLAMPED to same adaptive bounds used for daily eta.
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.14 — daily GeoNO2 + per-month adaptive eta_trop clamping [p1, p99]',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.14 (daily GeoNO2 + per-month adaptive eta_trop clamping [p1, p99])')
    ...
    print(f"geono2_v5.14  year={args.year}  instrument={args.instrument}")
    print(f"  daily GeoNO2 + per-month adaptive eta_trop clamping [p1, p99]")
