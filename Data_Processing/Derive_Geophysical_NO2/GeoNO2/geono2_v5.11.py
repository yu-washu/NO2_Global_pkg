"""
geono2_v5.11.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.11: Same bounds as v5.5 [5e-17, 8e-16] but with CLAMP instead of clip-to-NaN.
...
# Directories — v5.11: NO2col-v3 (AMF > 0.5 filter)
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.11/{year}/'


# ---------------------------------------------------------------------------
# Seasonal eta_trop clamping bounds
# ---------------------------------------------------------------------------
# Same bounds as v5.5 — but clamped instead of clipped to NaN
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.11 — daily GeoNO2 + eta_trop clamping (v5.5 bounds)',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.11 (daily GeoNO2 + eta_trop clamping)')
    ...
    print(f"geono2_v5.11  year={args.year}  instrument={args.instrument}")
