...
def load_yearly_data_minimal(year):
    ...
    omi_dir = f'/my-projects2/1.project/NO2_col/OMI_KNMI-v2/{year}/'
    
    tropomi_dir = '/my-projects2/1.project/NO2_col/TROPOMI/'
    try:
        ...
        # Load OMI yearly data without chunking
        print("  Loading OMI annual data...")
        omi_file = omi_dir + f"yearly/OMI_KNMI_Regrid_{year}_{qcstr}.nc"
        # omi_file = tropomi_dir + "yearly/TROPOMI_OMI_Regrid_2018.nc" # for 2018
        
        with xr.open_dataset(omi_file, engine='netcdf4') as yearly_omi:
            yearly_omi_NO2col_gcshape = yearly_omi['NO2_tot_gcshape'].values.astype('float32')
        
        print(f"✓ Successfully loaded annual data for {year}")
        print(f"  GCHP shape: {yearly_gchp_NO2col.shape}")
        print(f"  OMI shape: {yearly_omi_NO2col_gcshape.shape}")
        
        return yearly_gchp_NO2col, yearly_omi_NO2col_gcshape
        
    except Exception as e:
        print(f"✗ Error loading annual data for {year}: {str(e)}")
        raise

def process_single_day_minimal(year, month, day, yearly_gchp_NO2col, yearly_omi_NO2col_gcshape):
    """
    ...
    # File paths
    omi_file = omi_dir + f'daily/OMI_KNMI_Regrid_{year}{month:02d}{day:02d}_{qcstr}.nc'
    ...
    if not all(os.path.exists(f) for f in [omi_file, gchp_daily_path, gchp_3hours_path]):
        return None
    
    try:
        # Load TROPOMI data without chunking to avoid warnings
        with xr.open_dataset(omi_file, engine='netcdf4') as omi:
            omi = omi.squeeze()
            NO2_tot_gcshape = omi['NO2_tot_gcshape'].values.astype('float32')
            NO2_tot = omi['NO2_tot'].values.astype('float32')
            lat_coords = omi['lat'].values
            lon_coords = omi['lon'].values
        ...
        # Calculate geophysical NO2 with proper quality filtering
        NO2_geophysical = np.where(((NO2_tot_gcshape > 0) | (NO2_tot_gcshape <= 0)), 
                                  NO2_tot_gcshape * gchp_eta, 
                                  np.nan).astype('float32')
        
        # Debug: Print quality filtering statistics
        total_pixels = NO2_tot_gcshape.size
        valid_omi = np.sum(NO2_tot_gcshape > 0)
        nan_omi = np.sum(np.isnan(NO2_tot_gcshape))
        zero_or_neg_omi = np.sum(NO2_tot_gcshape <= 0)
        ...
        if day <= 3:  # Only print for first few days to avoid spam
            ...
            print(f"      OMI > 0: {valid_omi:,} ({100*valid_omi/total_pixels:.1f}%)")
            print(f"      OMI NaN: {nan_omi:,} ({100*nan_omi/total_pixels:.1f}%)")  
            print(f"      OMI ≤ 0: {zero_or_neg_omi:,} ({100*zero_or_neg_omi/total_pixels:.1f}%)")
...
def calculate_unbiased_model_average(year, current_month, candidate_months=None):
    """
    Calculate average GCHP NO2col for months that have OMI observations.
    candidate_months: list of months to consider (default: all months except current_month)
    Returns (ref_gchp_average, ref_omi_average, months_used) or (None, None, []) if no valid months found.
    """
    ...
    print(f"    Checking OMI monthly files in: {omi_dir}monthly/")
    print(f"    Checking GCHP monthly files in: {gchp_dir}monthly/")

    # Initialize accumulators for GCHP and OMI averages over reference months
    gchp_sum = None
    omi_sum = None
    ...
    for month in candidate_months:
        # Check if OMI-KNMI monthly file exists (try different naming patterns)
        omi_monthly_patterns = [
            omi_dir + f"monthly/OMI_KNMI_Regrid_{year}{month:02d}_Monthly_{qcstr}.nc",
            omi_dir + f"monthly/OMI_KNMI_Regrid_{year}{month:02d}_{qcstr}.nc",
            omi_dir + f"monthly/OMI_KNMI_Regrid_{year}{month:02d}.nc"
        ]

        omi_monthly_file = None
        for pattern in omi_monthly_patterns:
            if os.path.exists(pattern):
                omi_monthly_file = pattern
                break

        if omi_monthly_file is None:
            print(f"    Month {month:02d}: No OMI monthly file found")
            continue
        ...
        try:
            ...
            # Load OMI data — both for valid-pixel mask and for the OMI reference average
            with xr.open_dataset(omi_monthly_file, engine='netcdf4') as omi_ds:
                if 'NO2_tot_gcshape' in omi_ds:
                    omi_no2 = omi_ds['NO2_tot_gcshape'].values
                elif 'NO2_tot' in omi_ds:
                    omi_no2 = omi_ds['NO2_tot'].values
                else:
                    print(f"No recognized NO2 variable found")
                    continue

                valid_omi_mask = (omi_no2 > 0) & (~np.isnan(omi_no2))
                valid_count = np.sum(valid_omi_mask)

                if valid_count == 0:
                    print(f"No valid OMI pixels")
                    continue

                print(f"({valid_count:,} valid OMI pixels)", end=' ')
            ...
            if gchp_sum is None:
                ...
                omi_sum = np.zeros_like(gchp_no2col, dtype='float64')
                valid_pixel_count = np.zeros_like(gchp_no2col, dtype='int32')

            # Accumulate GCHP and OMI only where OMI has valid observations
            gchp_sum = np.where(valid_omi_mask,
                                gchp_sum + gchp_no2col.astype('float64'),
                                gchp_sum)
            omi_sum = np.where(valid_omi_mask,
                               omi_sum + omi_no2.astype('float64'),
                               omi_sum)
            valid_pixel_count = np.where(valid_omi_mask,
                                         valid_pixel_count + 1,
                                         valid_pixel_count)
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ref_gchp_average = np.where(valid_pixel_count > 0,
                                    ...
                                    np.nan).astype('float32')
        ref_omi_average = np.where(valid_pixel_count > 0,
                                   omi_sum / valid_pixel_count,
                                   np.nan).astype('float32')
...
def fill_missing_values_unbiased(year, current_month, geophysical_NO2, monthly_gchp_no2col,
    ...
    Within each tier, where ref_OMI is still NaN at a pixel (never observed
    in that tier's window), yearly OMI is used as a pixel-level fallback for
    the OMI term only (ref_GCHP must be valid for a fill to be applied).
    """
    ...
    for label, candidates, min_months in fallback_tiers:
        ...
        gchp_avg, omi_avg, months_used = calculate_unbiased_model_average(
        ...
        # Pixel-level OMI reference: seasonal avg where available, yearly elsewhere
        nan_omi = np.isnan(omi_avg)
        n_yearly_fb = int((still_needed & nan_omi).sum())
        if n_yearly_fb:
            print(f"    ref_OMI NaN at {n_yearly_fb:,} needed pixels → yearly fallback")
        omi_ref = np.where(nan_omi, yearly_omi_NO2col_gcshape,
                           omi_avg).astype('float32')
        ...
        # fill = scale × omi_ref × surface_ratio
        fill_this = (scale * omi_ref * ratio).astype('float32')
    ...
    # Final fallback: pixels still missing AND yearly OMI is also NaN → use monthly GCHP surface NO2 directly
    no_yearly_omi = np.isnan(yearly_omi_NO2col_gcshape)
    gchp_fallback_mask = (np.isnan(geophysical_NO2) & np.isnan(fill_values)
                          & no_yearly_omi & np.isfinite(gchp_no2))
    fill_values = np.where(gchp_fallback_mask, gchp_no2, fill_values)
    n_gchp_fallback = int(gchp_fallback_mask.sum())
    if n_gchp_fallback:
        print(f"  GCHP direct fallback: filled {n_gchp_fallback:,} pixels where yearly OMI is NaN")
...
PLOT_VARS = ['gap_GeoNO2', 'filled_GeoNO2', 'gchp_NO2']
PLOT_LABELS = {
...
def process_monthly_data_streaming(year, month, yearly_gchp_NO2col, yearly_omi_NO2col_gcshape,
                                   make_plots=True):
    """
    ...
    for day in range(1, days_in_month + 1):
        ...
        daily_result = process_single_day_minimal(year, month, day, 
                                                 yearly_gchp_NO2col, yearly_omi_NO2col_gcshape)
        ...
        if valid_day_count == 0:
            ...
            for var in ['geophysical_NO2', 'gchp_NO2', 'gchp_HNO3', 'gchp_PAN', 
                       'gchp_alkylnitrates', 'gchp_NO2col', 'omi_NO2_tot', 
                       'omi_NO2_tot_gcshape']:
    ...
    final_monthly = xr.Dataset({
        ...
        'omi_NO2_tot': (['lat', 'lon'], monthly_means['omi_NO2_tot']),
        'omi_NO2_tot_gcshape': (['lat', 'lon'], monthly_means['omi_NO2_tot_gcshape'])
    }, coords={
        'lat': lat_coords,
        'lon': lon_coords,
    ...
    final_monthly.attrs.update({
        ...
        'source': 'OMI-KNMI 13x24km + GCHP c180',
    ...
    OutDir = f'/my-projects2/1.project/GeoNO2-v3/{year}/'
...
def process_yearly_average_minimal(year):
    ...
    monthly_dir = f'/my-projects2/1.project/GeoNO2-v3/{year}/'
...
def process_single_month_minimal(year, month, make_plots=True):
    ...
    try:
        yearly_gchp_NO2col, yearly_omi_NO2col_gcshape = load_yearly_data_minimal(year)
    except Exception as e:
        return False
    ...
    # Cleanup yearly data
    del yearly_gchp_NO2col, yearly_omi_NO2col_gcshape
    gc.collect()
...
def plot_only(year, month=None):
    ...
    base_dir = f'/my-projects2/1.project/GeoNO2-v3/{year}/'
