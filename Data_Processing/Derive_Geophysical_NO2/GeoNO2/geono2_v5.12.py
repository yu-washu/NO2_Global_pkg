"""
geono2_v5.12.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.12: Same bounds as v5.6 [1e-16, 8e-16] but with CLAMP instead of clip-to-NaN.
...
# Directories — v5.12: NO2col-v3 (AMF > 0.5 filter)
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.12/{year}/'


# ---------------------------------------------------------------------------
# Seasonal eta_trop clamping bounds
# ---------------------------------------------------------------------------
# Same bounds as v5.6 — but clamped instead of clipped to NaN
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.12 — daily GeoNO2 + eta_trop clamping (v5.6 bounds)',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.12 (daily GeoNO2 + eta_trop clamping)')
    ...
    print(f"geono2_v5.12  year={args.year}  instrument={args.instrument}")
