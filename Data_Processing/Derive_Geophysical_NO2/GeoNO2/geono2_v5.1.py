"""
geono2_v5.1.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.1 change: monthly-eta approach.
  GeoNO2 is computed from monthly-averaged inputs (not daily eta):
    monthly_eta = mean(gchp_surface) / mean(gchp_column)
    GeoNO2 = mean(SatColNO2) × monthly_eta

Per-month output variables
--------------------------
  filled_SatColNO2_tot_gcshape   [gap-filled with GCHP col, Tier 1-4]
  filled_SatColNO2_trop_gcshape  [gap-filled with GCHP col, Tier 1-4]
  filled_SatColNO2_tot           [gap-filled with GCHP col, Tier 1-4]
  filled_SatColNO2_trop          [gap-filled with GCHP col, Tier 1-4]
  filled_GeoNO2_tot  = mean(SatColNO2_tot_gcshape) × monthly_eta_tot
  filled_GeoNO2_trop = mean(SatColNO2_trop_gcshape) × monthly_eta_trop
  filled_GeoNO2_trop_TM5 = mean(SatColNO2_trop) × monthly_eta_trop
...
-----------------------------
  fill = (monthly_GCHP_col / ref_GCHP_col) × ref_SAT
  Tier 4 fallback: fill = monthly_GCHP_col  (where yearly_sat is also missing)
  GeoNO2 gap-fill: fill = (GCHP/ref_GCHP) × ref_SAT × monthly_eta.
"""
import os
import gc
import calendar
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.1/{year}/'
...
for _v in ['filled_GeoNO2_tot', 'filled_GeoNO2_trop', 'filled_GeoNO2_trop_TM5','gchp_NO2']:
    VMAX[_v] = 15
for _v in ['filled_SatColNO2_tot_gcshape',
           'filled_SatColNO2_tot',
           'filled_SatColNO2_trop_gcshape', 
...
PLOT_LAYOUT = [
    ['filled_SatColNO2_tot_gcshape', 'filled_SatColNO2_trop_gcshape', 'gchp_NO2'                       ],
    ['filled_SatColNO2_tot_gcshape', 'filled_SatColNO2_trop_gcshape', 'filled_GeoNO2_tot'              ],
    ['filled_SatColNO2_tot',         'filled_SatColNO2_trop',         'filled_GeoNO2_trop'             ],
    ['filled_SatColNO2_tot',         'filled_SatColNO2_trop',         'filled_GeoNO2_trop_TM5'        ],
    [None,                           None,                            'gchp_eta_tot'                   ],
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    # ── Monthly-eta GeoNO2: mean(SatCol) × monthly_eta ────────────────
    # v5.1: compute monthly_eta = mean(surface) / mean(column), then multiply.
    ...
    # GeoNO2_tot = mean(SatColNO2_tot_gcshape) × monthly_eta_tot
    sat_tot_gcshape_mean = means.get('SatColNO2_tot_gcshape')
    if sat_tot_gcshape_mean is not None and monthly_eta_tot is not None:
        GeoNO2_tot = np.where(
            ~np.isnan(sat_tot_gcshape_mean),
            sat_tot_gcshape_mean * monthly_eta_tot, np.nan
        ).astype('float32')
        print("  Gap-filling GeoNO2_tot...")
        if monthly_gchp_tot is not None:
            filled_GeoNO2_tot = fill_missing(
                year, month, instrument, 'tot',
                GeoNO2_tot, monthly_gchp_tot,
                yearly_refs.get('sat_tot'),
        ...
        filled_GeoNO2_tot = None

    # GeoNO2_trop = mean(SatColNO2_trop_gcshape) × monthly_eta_trop
    sat_trop_gcshape_mean = means.get('SatColNO2_trop_gcshape')
    if sat_trop_gcshape_mean is not None and monthly_eta_trop is not None:
        GeoNO2_trop = np.where(
            ~np.isnan(sat_trop_gcshape_mean),
            sat_trop_gcshape_mean * monthly_eta_trop, np.nan
        ).astype('float32')
        print("  Gap-filling GeoNO2_trop...")
        if monthly_gchp_trop is not None:
            filled_GeoNO2_trop = fill_missing(
                year, month, instrument, 'trop',
                GeoNO2_trop, monthly_gchp_trop,
                yearly_refs.get('sat_trop'),
        ...
        filled_GeoNO2_trop = None

    # GeoNO2_trop_TM5 = mean(SatColNO2_trop) × monthly_eta_trop
    sat_trop_raw_mean = means.get('SatColNO2_trop')
    if sat_trop_raw_mean is not None and monthly_eta_trop is not None:
        GeoNO2_trop_TM5 = np.where(
            ~np.isnan(sat_trop_raw_mean),
            sat_trop_raw_mean * monthly_eta_trop, np.nan
        ).astype('float32')
        print("  Gap-filling GeoNO2_trop_TM5...")
        if monthly_gchp_trop is not None:
            filled_GeoNO2_trop_TM5 = fill_missing(
                year, month, instrument, 'trop',
                GeoNO2_trop_TM5, monthly_gchp_trop,
...
def main():
    parser = argparse.ArgumentParser(description='GeoNO2 v5.1 (monthly-eta, TROPOMI + OMI)')
    ...
    print(f"geono2_v5.1  year={args.year}  instrument={args.instrument}")
