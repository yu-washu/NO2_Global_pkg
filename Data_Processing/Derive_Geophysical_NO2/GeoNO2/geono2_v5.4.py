"""
geono2_v5.4.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.4: Same as v5.3 but with tighter eta_trop clipping bounds [2e-17, 1e-15] (approx 5th-95th percentile of eta_trop distribution in 2023 Jan).
  ...
  constant bounds [2e-17, 1e-15]), average daily GeoNO2 to monthly, then gap-fill.
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.4/{year}/'
...
ETA_TROP_MIN = 2e-17           # constant year-round
ETA_TROP_MAX = 1e-15           # constant year-round
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    ds_out = xr.Dataset(
        ...
        attrs={
            ...
            'version':     'v5.4 — daily GeoNO2 + tighter eta_trop clipping [2e-17, 1e-15]',
...
def main():
    parser = argparse.ArgumentParser(
        description='GeoNO2 v5.4 (daily GeoNO2 + eta_trop clipping + eta_trop clipping)')
    ...
    print(f"geono2_v5.4  year={args.year}  instrument={args.instrument}")
