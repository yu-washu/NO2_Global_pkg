...
# ==========================================================================
# GeoNO2 v5.1 Full Pipeline
#   Step 1: Submit GeoNO2 v5.1 monthly jobs (one per year-month)
...
GEONO2_SCRIPT="${BASEDIR}/Derive_Geophysical_NO2/GeoNO2/geono2_v5.1.py"
EVAL_SCRIPT="${BASEDIR}/Model_Evaluation_pkg/compr/compr_geo_gchp_global_v51.py"
PLOT_SCRIPT="${BASEDIR}/Model_Evaluation_pkg/compr/plot_scatter_global_v51.py"
LOGDIR="logs_v51"; mkdir -p "$LOGDIR"

GROUP="/yany1/GeoNO2_v51"
...
# ── Step 1: Submit GeoNO2 v5.1 monthly jobs ──────────────────────────────
echo ""
echo "========================================"
echo "  Step 1: Submitting GeoNO2 v5.1 jobs"
echo "========================================"
...
for year in $(seq $START_YEAR $END_YEAR); do
  for month in {1..12}; do
    ...
    JOB_NAME="GeoNO2v51_${year}${MONTH_STR}"
...
# ── Build dependency string: wait for ALL GeoNO2 jobs to finish ──────────
# "done(name)" triggers when the job completes (exit 0).
# Use "ended(name)" if you want to proceed even if some months fail.
DEP_STR=""
...
for year in $(seq $START_YEAR $END_YEAR); do
  ...
  bsub -q rvmartin \
       ...
       -a "docker(1yuyan/netcdf-mpi:latest)" \
       -o "${LOGDIR}/Eval_v51_${year}.out" \
       bash -lc $'\n'"\
. /opt/conda/bin/activate && \
/bin/bash && \
echo \"[\$(hostname)] Evaluation v5.1: ${year}\"
echo \"Start: \$(date)  Job: \$LSB_JOBID\"
...
"
  ...
  bsub -q rvmartin \
       ...
       -R "rusage[mem=${MEM_EVAL}]" \
       -a "docker(1yuyan/netcdf-mpi:latest)" \
       -o "${LOGDIR}/Plot_v51_${year}.out" \
       bash -lc $'\n'"\
. /opt/conda/bin/activate && \
/bin/bash && \
echo \"[\$(hostname)] Plotting v5.1: ${year}\"
echo \"Start: \$(date)  Job: \$LSB_JOBID\"
...
echo "  Evaluation:  Eval_v51_* (runs after all GeoNO2 jobs finish)"
echo "  Plots:       Plot_v51_* (runs after evaluation finishes)"
echo ""
echo "  Monitor with:  bjobs -g ${GROUP}"
echo "  Or:            bjobs -J 'GeoNO2v51_*'"
echo "  Eval/Plot:     bjobs -J 'Eval_v51_*' ; bjobs -J 'Plot_v51_*'"
echo "  Logs:          ${LOGDIR}/"
