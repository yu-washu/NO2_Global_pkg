"""
geono2_v5.py — merged GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).
...
--------------------------
  ...
  filled_GeoNO2_tot  = gap-filled from per-day mean(SatColNO2_tot_gcshape × eta_tot)
  filled_GeoNO2_trop = gap-filled from per-day mean(SatColNO2_trop_gcshape × eta_trop)
...
-----------------------------
  ...
  GeoNO2 is accumulated per-day then gap-filled with fill = (GCHP/ref_GCHP) × ref_SAT × eta.
"""
import os
import gc
import calendar
...
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5/{year}/'
...
def process_single_day(year, month, day, instrument):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        eta_tot  = np.where(col_tot  > 0, sfc / col_tot,  0).astype('float32')
        eta_trop = (np.where(col_trop > 0, sfc / col_trop, 0).astype('float32')
                    if col_trop is not None else None)

    NO2_tot_gcshape  = sat['NO2_tot_gcshape']
    NO2_trop_gcshape = sat['NO2_trop_gcshape']

    GeoNO2_tot = np.where(~np.isnan(NO2_tot_gcshape),
                          NO2_tot_gcshape * eta_tot,
                          np.nan).astype('float32') if NO2_tot_gcshape is not None else None

    GeoNO2_trop = (np.where(~np.isnan(NO2_trop_gcshape),
                             NO2_trop_gcshape * eta_trop,
                             np.nan).astype('float32')
                   if (NO2_trop_gcshape is not None and eta_trop is not None) else None)

    return {
        'lat': sat['lat'],
        'lon': sat['lon'],
        'GeoNO2_tot':          GeoNO2_tot,
        'GeoNO2_trop':         GeoNO2_trop,
        'SatColNO2_tot':       sat['NO2_tot'],
        'SatColNO2_trop':      sat['NO2_trop'],
        'SatColNO2_tot_gcshape': NO2_tot_gcshape,
        'SatColNO2_trop_gcshape': NO2_trop_gcshape,
        'gchp_NO2':            sfc,
        ...
        'gchp_eta_tot':        eta_tot,
        'gchp_eta_trop':       eta_trop,
    }
...
PLOT_LAYOUT = [
    ...
    ['filled_SatColNO2_tot',         'filled_SatColNO2_trop',         'gchp_eta_tot'                   ],
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    # ── Gap-fill GeoNO2 from per-day monthly means (like v3) ─────────────
    eta_tot  = means.get('gchp_eta_tot')
    eta_trop = means.get('gchp_eta_trop')

    if 'GeoNO2_tot' in means and monthly_gchp_tot is not None and eta_tot is not None:
        print("  Gap-filling GeoNO2_tot...")
        filled_GeoNO2_tot = fill_missing(
            year, month, instrument, 'tot',
            means['GeoNO2_tot'], monthly_gchp_tot,
            yearly_refs.get('sat_tot'),
            surface_ratio=eta_tot,
        )
    else:
        filled_GeoNO2_tot = means.get('GeoNO2_tot')

    if 'GeoNO2_trop' in means and monthly_gchp_trop is not None and eta_trop is not None:
        print("  Gap-filling GeoNO2_trop...")
        filled_GeoNO2_trop = fill_missing(
            year, month, instrument, 'trop',
            means['GeoNO2_trop'], monthly_gchp_trop,
            yearly_refs.get('sat_trop'),
            surface_ratio=eta_trop,
        )
    else:
        filled_GeoNO2_trop = means.get('GeoNO2_trop')
    ...
    # GeoNO2: per-day daily_eta variants
    ...
    _add('gchp_eta_tot',      means.get('gchp_eta_tot'))
    _add('gchp_eta_trop',     means.get('gchp_eta_trop'))
...
def main():
    parser = argparse.ArgumentParser(description='GeoNO2 v4 (TROPOMI + OMI, tot + trop)')
    ...
    print(f"geono2_v5  year={args.year}  instrument={args.instrument}")
