...
ECF_max, sza_max, QAFlag, RowAnomalyFlag = 0.3, 75, 0, 0
...
def load_yearly_data_minimal(year):
    ...
    omi_dir = f'/my-projects2/1.project/NO2_col/OMI-MINDS/{year}/'
    ...
    try:
        ...
        omi_file = omi_dir + f"yearly/OMI-MINDS_Regrid_{year}_{qcstr}.nc"
...
def process_single_day_minimal(year, month, day, yearly_gchp_NO2col, yearly_omi_NO2col_gcshape):
    ...
    # File paths
    omi_file = omi_dir + f'daily/OMI-MINDS_Regrid_{year}{month:02d}{day:02d}_{qcstr}.nc'
    ...
    try:
        ...
        return {
            ...
            'omi_NO2_tot': NO2_tot,
            'omi_NO2_tot_gcshape': NO2_tot_gcshape,
            'lat': lat_coords,
            'lon': lon_coords
        }
...
def calculate_unbiased_model_average(year, current_month):
    ...
    for month in range(1, 13):
        ...
        # Check if OMI-MINDS monthly file exists (try different naming patterns)
        omi_monthly_patterns = [
            omi_dir + f"monthly/OMI-MINDS_Regrid_{year}{month:02d}_Monthly_{qcstr}.nc",
            omi_dir + f"monthly/OMI-MINDS_Regrid_{year}{month:02d}_{qcstr}.nc",
            omi_dir + f"monthly/OMI-MINDS_Regrid_{year}{month:02d}.nc"
        ]
    ...
    if not months_with_data:
        ...
        print(f"    - OMI monthly files don't exist or use different naming")
        print(f"    - GCHP monthly files don't exist or use different naming") 
        print(f"    - All OMI monthly files contain only invalid data")
        return None
...
def fill_missing_values_unbiased(year, current_month, geophysical_NO2, monthly_gchp_no2col, 
    ...
    monthly_model / (average of model from months with OMI data) × yearly_satellite × (surface/column_ratio)
    """
    print("  Filling missing values using unbiased temporal scaling...")
    
    # Calculate unbiased model average from months with OMI observations
    ...
    if unbiased_gchp_average is None:
        ...
        print("  This indicates OMI had no valid observations for this year")
    ...
    fill_values = (scale_factors * yearly_omi_NO2col_gcshape * ratio).astype('float32')
...
def process_monthly_data_streaming(year, month, yearly_gchp_NO2col, yearly_omi_NO2col_gcshape):
    ...
    min_observations = max(1, valid_day_count // 4)  # Require at least 25% of days
