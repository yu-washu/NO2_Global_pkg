"""
geono2_v5.2.py — GeoNO2 + SatColNO2 gap-fill script (TROPOMI & OMI).

v5.2: Cooper-lite column-ratio approach (no BL/FT separation).
  Instead of GeoNO2 = SatCol × eta, compute:
    R = mean(SatCol) / mean(GCHP_col)   [dimensionless column ratio]
    GeoNO2 = mean(GCHP_surface) × R_filled

  Gap-filling targets the dimensionless ratio R directly:
    Tiers 1-3: R_fill = ref_R from seasonal cascade (no GCHP scaling needed)
    Tier 4:    R = 1.0 (use model surface as-is)

Per-month output variables
--------------------------
  filled_SatColNO2_{tot,trop}_{gcshape,raw}  [gap-filled, Tier 1-4]
  column_ratio_{tot,trop,trop_TM5}           [pre-fill monthly R]
  filled_column_ratio_{tot,trop,trop_TM5}    [gap-filled R]
  filled_GeoNO2_{tot,trop,trop_TM5}          [= gchp_NO2 × filled_R]
  gchp_NO2, gchp_HNO3, gchp_PAN, gchp_alkylnitrates
  gchp_NO2col_tot, gchp_NO2col_trop, gchp_eta_tot, gchp_eta_trop

SatColNO2 gap-fill (Tiers 1-3): fill = (GCHP_col / ref_GCHP) × ref_SAT
  Tier 4: fill = GCHP_col
Column-ratio gap-fill (Tiers 1-3): R_fill = ref_SAT / ref_GCHP (yearly fallback)
  Tier 4: R = 1.0
"""
import os
import gc
...
TROPOMI_COL_DIR = '/my-projects2/1.project/NO2col-v2/TROPOMI/{year}/'
OMI_COL_DIR     = '/my-projects2/1.project/NO2col-v2/OMI_KNMI/{year}/'
OUT_ROOT        = '/my-projects2/1.project/GeoNO2-v5.2/{year}/'
...
def fill_missing(year, current_month, instrument, column_type,
    ...
    SatColNO2 fill (surface_ratio=None):
        Tiers 1-3: fill = (monthly_GCHP_col / ref_GCHP_col) × ref_SAT
...
def fill_ratio_missing(year, current_month, instrument, column_type,
                       monthly_ratio, yearly_ratio=None, sat_var=None):
    """
    Gap-fill column ratio R = Omega_sat / Omega_model.

    Tiers 1-3: R_fill = ref_sat / ref_gchp from seasonal cascade
               (with yearly-ratio fallback per pixel where seasonal is NaN)
    Tier 4:    R = 1.0  (use model surface as-is)
    """
    fallback_tiers = [
        (f'Tier 1 — same season ({get_season(current_month)})',
    ...
    ratio_missing = ~np.isfinite(monthly_ratio)
    fill_values   = np.full(monthly_ratio.shape, np.nan, dtype='float32')
    total         = monthly_ratio.size
    n_orig_miss   = int(ratio_missing.sum())

    for label, candidates, min_months in fallback_tiers:
        still_needed = ratio_missing & np.isnan(fill_values)
        if not still_needed.any():
            break
        print(f"    {label}: {int(still_needed.sum()):,} pixels")
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ref_ratio = np.where((rg > 0) & np.isfinite(rs),
                                 (rs / rg).astype('float32'), np.nan)

        # Yearly-ratio fallback where seasonal ref is NaN
        if yearly_ratio is not None:
            nan_ref = np.isnan(ref_ratio)
            ref_ratio = np.where(nan_ref, yearly_ratio, ref_ratio)

        applied     = still_needed & np.isfinite(ref_ratio)
        fill_values = np.where(applied, ref_ratio, fill_values)
        n_still     = int(np.sum(ratio_missing & np.isnan(fill_values)))
        print(f"      -> filled {int(applied.sum()):,}; {n_still:,} still missing")

    # Tier 4: R = 1.0 (model as-is)
    still_needed = ratio_missing & np.isnan(fill_values)
    if still_needed.any():
        n_fb = int(still_needed.sum())
        fill_values = np.where(still_needed, 1.0, fill_values)
        if n_fb:
            print(f"    Tier 4 — model direct (R=1): filled {n_fb:,} pixels")

    result = np.where(ratio_missing, fill_values, monthly_ratio).astype('float32')

    n_filled = int((ratio_missing & np.isfinite(fill_values)).sum())
    n_still  = int((~np.isfinite(result)).sum())
    print(f"    Ratio fill [{column_type}]: orig_missing={n_orig_miss:,}  "
          f"filled={n_filled:,}  still_missing={n_still:,}  total={total:,}")
    return result
...
PLOT_VARS = [
    ...
    'column_ratio_trop',            'filled_column_ratio_trop',
...
PLOT_LABELS = {
    ...
    'column_ratio_trop':            'R trop (before fill)',
    'filled_column_ratio_trop':     'R trop (filled)',
...
for _v in ['gchp_eta_tot', 'gchp_eta_trop']:
    VMAX[_v] = 1e-15
for _v in ['column_ratio_trop', 'filled_column_ratio_trop']:
    VMAX[_v] = 3.0

# Fixed 6×3 plot layout
PLOT_LAYOUT = [
    ...
    ['column_ratio_trop',            'filled_column_ratio_trop',      'gchp_eta_tot'                   ],
...
# Variables accumulated day-by-day
_DAY_VARS = [
...
def process_monthly_streaming(year, month, instrument, yearly_refs, make_plots=True):
    ...
    # ── Cooper-lite: column-ratio approach ────────────────────────────
    # R = Omega_sat / Omega_model (dimensionless)
    # GeoNO2 = GCHP_surface × R_filled
    mean_sfc      = means.get('gchp_NO2')
    mean_col_tot  = means.get('gchp_NO2col_tot')
    mean_col_trop = means.get('gchp_NO2col_trop')

    # Monthly eta (for output / reference only)
    with np.errstate(divide='ignore', invalid='ignore'):
    ...
    # Compute column ratios from monthly means
    sat_tot_gc  = means.get('SatColNO2_tot_gcshape')
    sat_trop_gc = means.get('SatColNO2_trop_gcshape')
    sat_trop_raw = means.get('SatColNO2_trop')

    with np.errstate(divide='ignore', invalid='ignore'):
        R_tot = (np.where((mean_col_tot > 0) & np.isfinite(sat_tot_gc),
                          sat_tot_gc / mean_col_tot, np.nan).astype('float32')
                 if (sat_tot_gc is not None and mean_col_tot is not None) else None)
        R_trop = (np.where((mean_col_trop > 0) & np.isfinite(sat_trop_gc),
                           sat_trop_gc / mean_col_trop, np.nan).astype('float32')
                  if (sat_trop_gc is not None and mean_col_trop is not None) else None)
        R_trop_TM5 = (np.where((mean_col_trop > 0) & np.isfinite(sat_trop_raw),
                                sat_trop_raw / mean_col_trop, np.nan).astype('float32')
                       if (sat_trop_raw is not None and mean_col_trop is not None) else None)

    # Compute yearly ratios for fallback
    yearly_R_tot = yearly_R_trop = yearly_R_trop_raw = None
    yr_sat_tot  = yearly_refs.get('sat_tot')
    yr_sat_trop = yearly_refs.get('sat_trop')
    yr_sat_trop_raw = yearly_refs.get('sat_trop_raw')
    yr_gchp_tot  = yearly_refs.get('gchp_tot')
    yr_gchp_trop = yearly_refs.get('gchp_trop')
    with np.errstate(divide='ignore', invalid='ignore'):
        if yr_sat_tot is not None and yr_gchp_tot is not None:
            yearly_R_tot = np.where(yr_gchp_tot > 0, yr_sat_tot / yr_gchp_tot, np.nan).astype('float32')
        if yr_sat_trop is not None and yr_gchp_trop is not None:
            yearly_R_trop = np.where(yr_gchp_trop > 0, yr_sat_trop / yr_gchp_trop, np.nan).astype('float32')
        if yr_sat_trop_raw is not None and yr_gchp_trop is not None:
            yearly_R_trop_raw = np.where(yr_gchp_trop > 0, yr_sat_trop_raw / yr_gchp_trop, np.nan).astype('float32')

    # Gap-fill ratios
    filled_R_tot = filled_R_trop = filled_R_trop_TM5 = None
    if R_tot is not None:
        print("  Gap-filling column_ratio_tot...")
        filled_R_tot = fill_ratio_missing(
            year, month, instrument, 'tot', R_tot, yearly_R_tot)
    if R_trop is not None:
        print("  Gap-filling column_ratio_trop...")
        filled_R_trop = fill_ratio_missing(
            year, month, instrument, 'trop', R_trop, yearly_R_trop)
    if R_trop_TM5 is not None:
        print("  Gap-filling column_ratio_trop_TM5...")
        filled_R_trop_TM5 = fill_ratio_missing(
            year, month, instrument, 'trop', R_trop_TM5, yearly_R_trop_raw,
            sat_var='NO2_trop')

    # GeoNO2 = GCHP_surface × R_filled
    filled_GeoNO2_tot = (mean_sfc * filled_R_tot).astype('float32') \
        if (mean_sfc is not None and filled_R_tot is not None) else None
    filled_GeoNO2_trop = (mean_sfc * filled_R_trop).astype('float32') \
        if (mean_sfc is not None and filled_R_trop is not None) else None
    filled_GeoNO2_trop_TM5 = (mean_sfc * filled_R_trop_TM5).astype('float32') \
        if (mean_sfc is not None and filled_R_trop_TM5 is not None) else None
    ...
    # Column ratios (pre-fill + filled)
    _add('column_ratio_tot',             R_tot)
    _add('filled_column_ratio_tot',      filled_R_tot)
    _add('column_ratio_trop',            R_trop)
    _add('filled_column_ratio_trop',     filled_R_trop)
    _add('column_ratio_trop_TM5',        R_trop_TM5)
    _add('filled_column_ratio_trop_TM5', filled_R_trop_TM5)
    # GeoNO2 (Cooper-lite: S × R)
...
def main():
    parser = argparse.ArgumentParser(description='GeoNO2 v5.2 (Cooper-lite column-ratio, TROPOMI + OMI)')
    ...
    print(f"geono2_v5.2  year={args.year}  instrument={args.instrument}")
