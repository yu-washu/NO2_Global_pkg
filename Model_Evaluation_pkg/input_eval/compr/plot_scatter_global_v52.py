...
def load_and_process_data():
    annual_gchp_geo_file = os.path.join(
        data_dir, f'{species}_annual_{year}_obs{obs_version}_geono2-{geono2_version}.csv')
...
SAT_COLS = [
    ('filled_SatColNO2_trop_gcshape',   'SatColNO2_trop_gcshape (DU)'),
    ('filled_SatColNO2_tot_gcshape',    'SatColNO2_tot_gcshape (DU)'),
    ('filled_SatColNO2_trop',           'SatColNO2_trop (DU)'),
    ('filled_SatColNO2_tot',            'SatColNO2_tot (DU)'),
]
...
def create_summary_statistics(gchp_geo_df):
    print("\n" + "="*80)
    print("GLOBAL SUMMARY STATISTICS (GeoNO2 v5.2)")
    print("="*80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Global NO2 scatter plot evaluation (GeoNO2 v5.2)')
    ...
    print(f"Starting global NO2 evaluation and visualization (GeoNO2 v5.2) for year {year}...")
