...
GCHP_Dir = f'/my-projects2/1.project/GeoNO2-v5.2/{year}/'
...
def process_monthly(obs_data, gchp_coords):
    ...
    for month in sorted(grouped['mon'].unique()):
        ...
        with nc4.Dataset(fp, 'r') as ds:
            ...
            v_sat_trop_gc = _extract_nc4(ds, 'filled_SatColNO2_trop_gcshape')
            v_sat_tot_gc  = _extract_nc4(ds, 'filled_SatColNO2_tot_gcshape')
            v_sat_trop   = _extract_nc4(ds, 'filled_SatColNO2_trop')
            v_sat_tot    = _extract_nc4(ds, 'filled_SatColNO2_tot')
        ...
        monthly_rows.append(pd.DataFrame({
            ...
            'filled_SatColNO2_trop_gcshape': v_sat_trop_gc,
            'filled_SatColNO2_tot_gcshape':  v_sat_tot_gc,
            'filled_SatColNO2_trop':         v_sat_trop,
            'filled_SatColNO2_tot':          v_sat_tot,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Global NO2 evaluation (GeoNO2 v5.2)')
    ...
    print(f"Starting global NO2 evaluation (GeoNO2 v5.2) for year {year}...")
