...
def load_and_process_data():
    monthly_file = os.path.join(data_dir, f'{species}_monthly_{year}_obs{obs_version}.csv')
    annual_gchp_geo_file = os.path.join(data_dir, f'{species}_annual_{year}_obs{obs_version}.csv')
    annual_cooper_file = os.path.join(data_dir, f'{species}_annual_cooper_{year}_obs{obs_version}.csv')
...
def create_method_by_region_figure(gchp_geo_df, cooper_df, a4_orientation='landscape'):
    ...
    out_base = os.path.join(output_dir, f'{species}_by_region_{year}_obs{obs_version}')
